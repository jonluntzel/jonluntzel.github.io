...
<p align=center>
<sub>The cofactor expansion and its unrolled counterpart below relating the determinant to individual matrix entries. The intuition for how single entries in relation to others contribute to the full spectrum is often confusing.<sub> ok h </sub>
</p?>
...
Chains can neighbor free regions like in $1(23)4$ and we see the same chains at $1$ and $4$ in $1 x y,4$ and gaps that enumerate $2$-length selections $xy$ from  
```math
\lbrace 2,3 \rbace \times \lbrace 2,3 \rbrace = \begin{pmatrix}22 && 23 \\ 32 && 33\end{pmatrix}.
```
The analogy extends to multiple free regions or blocks - $1(23)(45)$.
