...
Given k blocks of dimensions $b_i$, let e and p be the unique dimensions of k blocks and their respective frequencies. The number of arrangements of the given blocks over n fixed elements (the # of such partial orders) is $$\prod_{i=1}^{k}\frac{n-(\sum_{j=1}^{k}b_j)-i)}{\prod_{k=1}^{|p|}p_{k}!}$$ . The denominator accounts for symmetries in block assignments (212 permutes on same-valued indexes are identical).
