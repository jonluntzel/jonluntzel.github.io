...
Chains can neighbor free regions like in 1(23)4 and we see the same gaps in free regions ({{1,x},{y,4}}) that enumerate 2-length selections xy from $${2,3}^{2}$$. Multiple free regions or blocks are fine - 1(23)(45) - as are larger blocks - (123)4(56). 
...
Given k blocks of dimensions $$b_i$$, let e and p be the unique dimensions of k blocks and their respective frequencies. The number of arrangements of the given blocks over n fixed elements (the # of such partial orders) is $$\prod_{i=1}^{k}(n-(\sum_{j=1}^{k}b_j)-i)/(prod(map(!,p)))$$ . The denominator accounts for symmetries in block assignments (212 permutes on same-valued indexes are identical).
