...
and exploit Schur's complement for invertible X to compute $\det(A) = \det(X)*\det(Y - Q . X^{-1} . P)$. Similar to the cofactor expansion we can recursively apply this definition to make headway on the 'why' ile matrices happen.
